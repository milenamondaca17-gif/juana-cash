...
class CobrarDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        self.input_monto_sec.setStyleSheet(f"background: {BG_MAIN}; color: {TEXT_MAIN}; font-size: 18px; padding: 5px; border: 1px solid {ACCENT_OFERTAS}; border-radius: 4px;")
        ...
        self.input_entrega.setStyleSheet(f"background: {BG_MAIN}; color: {TEXT_MAIN}; font-size: 20px; padding: 5px;")
        ...
        self.input_cupon.setStyleSheet(f"QLineEdit {{ background: {BG_MAIN}; border: 1px solid #8e44ad; border-radius: 6px; color: {TEXT_MAIN}; padding: 4px 8px; font-size: 13px; }}")
        ...
        QTimer.singleShot(100, self.input_entrega.setFocus)
...
class VentasScreen(QWidget):
    ...
    def buscar_cliente(self):
        ...
        lbl.setStyleSheet(f"color: {TEXT_MAIN}; font-size: 13px;")
        lay.addWidget(lbl)
        input_b = QLineEdit()
    ...
    def verificar_precio(self):
        ...
        input_b.setStyleSheet(f"QLineEdit {{ background: {BG_PANEL}; border: 2px solid #1abc9c; border-radius: 10px; padding: 10px; color: {TEXT_MAIN}; font-size: 16px; }}")
        lay.addWidget(input_b)
    ...
    def abrir_ingreso_departamento(self, depto):
        ...
        def actualizar_lista():
            ...
            lbl_montos.setStyleSheet(f"color: {TEXT_MAIN}; font-size: 13px;")
        ...
        input_monto.setStyleSheet(f"QLineEdit {{ background: {BG_MAIN}; border: 2px solid {depto['color']}; border-radius: 8px; padding: 0 16px; color: {TEXT_MAIN}; font-size: 20px; font-weight: bold; }}")
        input_frame = QFrame()
    ...
    def _crear_boton_celda(self, texto, color, bg, bg_hover):
        ...
        btn.setStyleSheet(
            ...
            "QPushButton:hover {"
            f" background: {bg_hover}; color: white; border: 1.5px solid {color}; }}"
            "QPushButton:pressed {"
            f" background: {bg_hover}; color: white; border: 1.5px solid {color}; }}"
        )
        return btn
    ...
    def actualizar_tabla(self):
        ...
        for i, item in enumerate(self.items_venta):
            ...
            if self.tabla.cellWidget(i, 4) is None:
                ...
                btn_menos = self._crear_boton_celda("−", "#f87171", "#2d0a0a", "#7f1d1d")
                btn_menos.clicked.connect(lambda _, idx=i: self.cambiar_cantidad(idx, -1))
                btn_layout.addWidget(btn_menos)
                btn_mas = self._crear_boton_celda("+", "#4ade80", "#052a12", "#14532d")
                ...
                btn_del = self._crear_boton_celda("🗑", "#f87171", "#1a0505", "#450e0e")
    ...
    def abrir_busqueda_avanzada(self):
        ...
        input_b.setFixedHeight(48)
        input_b.setStyleSheet(f"QLineEdit {{ background: {BG_PANEL}; border: 2px solid {ACCENT_BOTON}; border-radius: 10px; padding: 10px; color: {TEXT_MAIN}; font-size: 15px; }}")
        lay.addWidget(input_b)
