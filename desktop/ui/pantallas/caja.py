...
class CajaScreen(QWidget):
    ...
    def setup_ui(self):
        ...
        # Botones apertura/cierre + gastos
        btns = QHBoxLayout()
        ...
        self.tabla.setHorizontalHeaderLabels(["Ticket", "Total", "Método de pago", "Cajero", "Hora"])
    ...
    def abrir_caja(self):
        try:
            ...
            return
        try:
            r = requests.post(f"{API_URL}/caja/abrir", json={
            ...
            if r.status_code == 200:
                ...
                try:
                    requests.post(f"{API_URL}/sesiones/registrar", json={
                        ...
                        "detalle": f"Monto inicial: ${monto:.2f}"
                    }, timeout=3)
                except Exception:
                    pass
            else:
    ...
    def cerrar_caja(self):
        ...
        try:
            ...
            total_calculado = 0
            if r_hoy.status_code == 200:
                total_calculado = r_hoy.json().get("total_vendido", 0)
        except Exception:
            total_calculado = 0
        ...
        respuesta = QMessageBox.question(self, "🔒 Cerrar caja",
            f"RESUMEN DE CIERRE\n"
            f"{'═' * 30}\n"
            f"Total calculado: ${total_calculado:.2f}\n"
            f"Monto declarado: ${monto_cierre:.2f}\n"
            f"{color_dif} Diferencia: ${diferencia:.2f}\n\n"
            f"¿Confirmás el cierre?",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)

        if respuesta == QMessageBox.StandardButton.Yes:
            try:
                r = requests.post(f"{API_URL}/caja/cerrar/{self.turno_actual['id']}", json={
                ...
                if r.status_code == 200:
                    ...
                    try:
                        requests.post(f"{API_URL}/sesiones/registrar", json={
                            ...
                            "nombre_cajero": self.nombre_cajero,
                            "accion": "CIERRE_CAJA",
                            "detalle": f"Total: ${total_calculado:.2f} | Declarado: ${monto_cierre:.2f} | Dif: ${diferencia:.2f}"
                        }, timeout=3)
                    except Exception:
                        pass
                    ...
                    QMessageBox.information(self, "✅ Caja cerrada",
                        ...
                        f"{'✅ Sin diferencia' if abs(diferencia) < 1 else f'Diferencia: ${diferencia:.2f}'}")
    ...
    def registrar_gasto(self):
        ...
        lbl = QLabel("Descripción del gasto:")
        ...
        combo.setStyleSheet("""
            ...
            QComboBox QAbstractItemView { background: #0f3460; color: white; selection-background-color: #9b59b6; }
        """)
        lay.addWidget(combo)
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                for i, v in enumerate(ventas):
                    ...
                    self.tabla.setItem(i, 1, QTableWidgetItem(f"${float(v['total']):.2f}"))
                    ...
                    self.tabla.setItem(i, 3, QTableWidgetItem(v.get("cajero", "-")))
