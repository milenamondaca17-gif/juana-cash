...
class UsuariosScreen(QWidget):
    _usuarios_listos = pyqtSignal(list)

    def __init__(self):
        super().__init__()
        self.setup_ui()
        self._usuarios_listos.connect(self._aplicar_usuarios)
        ...
        def _fetch():
            try:
                ...
                if r.status_code == 200:
                    self._usuarios_listos.emit(r.json())
            except Exception:
                pass
        threading.Thread(target=_fetch, daemon=True).start()

    def _aplicar_usuarios(self, usuarios):
        ...
        if dialog.exec():
            ...
            try:
                ...
                if r.status_code == 200:
                    ...
                    QMessageBox.critical(self, "Error", "No se pudo crear el cajero")
            except Exception:
        ...
        if QMessageBox.question(self, "Confirmar", "¿Eliminar este cajero?") == QMessageBox.StandardButton.Yes:
            try:
                ...
                if r.status_code == 200:
                    ...
                    QMessageBox.critical(self, "Error", "No se pudo eliminar el cajero")
            except Exception:
                QMessageBox.critical(self, "Error", "No se puede conectar al servidor")